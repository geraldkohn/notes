...
### 假设有 5 个节点，网络分区了，变成2个节点和3个节点的集群，3个节点的集群一定会选出 Leader 吗？

* 假设 2 个节点的集群中间有 Leader，那么 Leader 的日志将不能被提交。因为需要提交到至少 2 个 Follower 上。另外 3 个节点的集群中间可能会选出 Leader。

* 假设 3 个节点的集群中间有 Leader，那么 Leader 的日志将正常提交。
...
### 是否可能某个entry已经存储到大多数节点，但是被新 Leader 覆盖掉
...
这里的问题在于：**判断是否要提交的唯一标准是“已同步的副本数量”**：数量超过了集群半数，则认为 应该提交，**没有考虑到任期信息，“任期不同但副本数量都超过集群半数”的情况是可能的**，即上面的 (d) 情况。 因此，为了避免这个问题，Raft 做了一个限制：**只有提交当前任期内的记录时， 才能用这种计算副本数量（counting replicas）的方式**。对应到图 8 中就是 (d) 时刻， `index=2,term=2` 的三个副本可以被合理覆盖，因为此时已经是 `term=3` 了。 一旦当前任期内的一个 entry 已这种方式被提交了，那所有之前的 entries 都会被 Log Matching Property 间接提交，对应图中的 (e) 场景。

## 参考

[etcd/raft 源码解析](https://arthurchiao.art/blog/raft-paper-zh/#633-%E8%A2%AB%E7%A7%BB%E9%99%A4%E7%9A%84%E8%8A%82%E7%82%B9%E4%B8%8D%E6%96%AD%E8%B6%85%E6%97%B6timeout%E5%B9%B6%E8%A7%A6%E5%8F%91%E9%80%89%E4%B8%BE)
